...
export const BASE_PATH = 'https://accounts.goto-products.com';
...
export const DEFAULT_CLIENT_ID = 'gojek:consumer:app';

// Static device fingerprint (X-E2) - captured from iOS device
export const DEFAULT_X_E2 = 'C3948F5848C29AB28455A1BFA32A8';
...
export interface DeviceConfig {
  ...
  sessionId: string;
  deviceToken?: string;
  xE2?: string;  // Device fingerprint
}

/**
 * Create default device config for iOS
 */
export function createIOSDeviceConfig(): DeviceConfig {
 ...
 */
export function createAndroidDeviceConfig(): DeviceConfig {
...
export class AuthApi {
   ...
   */
  private calculateXE3(body?: object): string {
    if (!body) {
  ...
  private getDefaultHeaders(): Record<string, string> {
    const headers: Record<string, string> = {
      ...
      'X-E2': this.deviceConfig.xE2 || DEFAULT_X_E2,
  ...
  async getLoginMethods(
    ...
    if (signatureHeaders?.X_E1) {
      headers['X-E1'] = signatureHeaders.X_E1;
    }

    const response = await this.axios.post<LoginMethodsResponse>(
  ...
  async initiateVerification(
    ...
    verificationMethod: 'goto_pin' | 'otp_sms' | 'otp_wa' | 'otp_email',
    phoneNumber: string,
    countryCode: string = '+62',
    flow: string = 'login_1fa',
    signatureHeaders?: { X_E1: string }
    ...
    const response = await this.axios.post<CVSInitiateResponse>(
      '/cvs/v1/initiate',
      body,
      { headers }
    );
  ...
  async verifyCode(
    verificationId: string,
    verificationMethod: 'goto_pin' | 'otp_sms' | 'otp_wa' | 'otp_email',
    data: { challenge_id: string; validation_jwt: string } | { otp: string; otp_token: string },
    flow: string = 'login_1fa',
    signatureHeaders?: { X_E1: string }
    ...
    const response = await this.axios.post<CVSVerifyResponse>(
      '/cvs/v1/verify',
      body,
      { headers }
    );
  ...
  async getAccountList(
    ...
    const response = await this.axios.post<AccountListResponse>(
  ...
  async exchangeToken(
    token: string,
    accountId: string,
    grantType: 'cvs' | 'auth_code' | 'onetap' = 'cvs',
    signatureHeaders?: { X_E1: string }
    ...
    const response = await this.axios.post<TokenExchangeResponse>(
      '/goto-auth/token',
      body,
      { headers }
    );
   ...
   */
  async ssoLogin(signatureHeaders?: { X_E1: string }): Promise<SSOLoginResponse> {
    const body = {
    ...
    const response = await this.axios.post<SSOLoginResponse>(
      '/goto-auth/sso/login',
      body,
      { headers }
    );
   ...
   */
  async logout(accessToken: string, signatureHeaders?: { X_E1: string }): Promise<void> {
    const headers: Record<string, string> = {
      ...this.getDefaultHeaders(),
